...
This document records the roadmap exposed by the MND/ALS review dogfood project. The MND-specific claims and rubrics belong in that project. The infrastructure patterns belong in Paperscope.
...
| Paper card | short summary, relevance note, key claims, methods, populations, limitations | yes, after review |
...
4. Add `systematic_review.knowledge_base` exporter.
5. Extend static HTML output to consume paper cards and clusters.
6. Document a Next.js/Vercel consumer pattern without making it the only supported UI.
7. Keep the MND project as regression dogfood for scale, messy metadata, rater disagreement, and private source access.

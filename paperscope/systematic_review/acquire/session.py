...
class Session:
    ...
    Two construction paths:
      - `Session.warmup(state_path, ezproxy_host)` opens a visible browser
        and waits for the user to complete institutional login. On exit,
        the state is dumped to `state_path` for subsequent reuse.
      - `Session.headless(state_path)` reuses a previously-dumped state
        without showing a window. Falls back to headed if no state file
        exists yet.
    """

...
Conventions match this skeleton: a library root (``--root`` or ``$PAPERLIBRARY_ROOT``, default
``~/PaperLibrary``) holding ``catalog.db``, ``pdfs/<cite_key>.pdf``, ``text/<cite_key>.txt``.
...
def _root(argv):
    ...
    return Path(r or os.environ.get("PAPERLIBRARY_ROOT") or Path.home() / "PaperLibrary")
...
def main():
    ...
    if not (root / "catalog.db").exists():
        sys.exit(f"no catalog.db under {root} (set --root or $PAPERLIBRARY_ROOT)")
    migrate(root)
